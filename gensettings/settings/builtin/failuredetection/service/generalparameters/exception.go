...
func (me Exceptions) MarshalHCL() (map[string]interface{}, error) {
	...
	if len(me) > 0 {
		...
		result["successForcingException"] = entries
	}
	return result, nil
}

func (me *Exceptions) UnmarshalHCL(decoder hcl.Decoder) error {
	if value, ok := decoder.GetOk("successForcingException"); ok {
		...
		for _, entryMap := range entrySet.List() {
			...
			entry := new(Exception)
			if err := entry.UnmarshalHCL(hcl.NewDecoder(decoder, "successForcingException", hash)); err != nil {
				return err
			}
			*me = append(*me, entry)
		}
