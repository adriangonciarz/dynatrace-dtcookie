...
type ResponseTimeAuto struct {
	...
	ResponseTimeAll        *ResponseTimeAutoAll        `json:"responseTimeAll"`        // Alert if the average response time of all user actions degrades beyond **both** the absolute and relative thresholds:
...
func (me *ResponseTimeAuto) Schema() map[string]*hcl.Schema {
	return map[string]*hcl.Schema{
		...
		"response_time_all": {
			Type:        hcl.TypeList,
			Description: "Alert if the average response time of all user actions degrades beyond **both** the absolute and relative thresholds:",
			Required:    true,
