...
type ResponseTimeFixed struct {
	...
	ResponseTimeAll        *ResponseTimeFixedAll     `json:"responseTimeAll"`        // Alert if the average response time of all requests degrades beyond this threshold:
	ResponseTimeSlowest    *ResponseTimeFixedSlowest `json:"responseTimeSlowest"`    // Alert if the average response time of the slowest 10% of requests degrades beyond this threshold:
...
func (me *ResponseTimeFixed) Schema() map[string]*hcl.Schema {
	return map[string]*hcl.Schema{
		...
		"response_time_all": {
			Type:        hcl.TypeList,
			Description: "Alert if the average response time of all requests degrades beyond this threshold:",
			Required:    true,
		...
		"response_time_slowest": {
			Type:        hcl.TypeList,
			Description: "Alert if the average response time of the slowest 10% of requests degrades beyond this threshold:",
			Required:    true,
