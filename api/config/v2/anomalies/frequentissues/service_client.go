...
type SettingsObject struct {
	...
	Value         *FrequentIssues `json:"value"`
}
...
func (cs *ServiceClient) Create(item *FrequentIssues) (string, error) {
	payload := v2common.SettingsObjectCreate{
		Value:         item,
		SchemaID:      "builtin:anomaly-detection.frequent-issues",
...
func (cs *ServiceClient) Update(id string, item *FrequentIssues) error {
...
func (cs *ServiceClient) Get(id string) (*FrequentIssues, error) {
	if len(id) == 0 {
...
func (cs *ServiceClient) List() ([]string, error) {
	...
	if bytes, err = cs.client.GET("/settings/objects?schemaIds=builtin%3Aanomaly-detection.frequent-issues&fields=objectId&pageSize=500", 200); err != nil {
		return nil, err
	}
